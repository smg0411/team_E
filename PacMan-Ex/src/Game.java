...
public class Game extends Canvas implements Runnable,KeyListener{
	...
	public Game() {
		...
		score = new Score("/scoreboard/scoreboard.txt");
	...
	private void render() { // tick에서 변화시킨 클래스를 실제로 보여줌
		...
		if(bs==null) { 
			createBufferStrategy(3); //버퍼 3개 --> 그림을 그리는 것 보다 화면에 표출하는 것이 빠른 현상 방지
			return;
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		else if(STATE==START) { // 엔터를 누르면 게임 시작
	...
	public void keyReleased(KeyEvent e) { // 팩맨의 이동을 멈춤
		if(e.getKeyCode()==KeyEvent.VK_RIGHT)
			pacman.right=false;
		if(e.getKeyCode()==KeyEvent.VK_LEFT)
			pacman.left=false;
